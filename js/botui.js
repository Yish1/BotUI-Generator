...
function(root, undefined) {
	...
	var BotUI = (function(id, opts) {
		...
		_fontAwesome = 'https://cmxz.top/Bot/js/_fontAwesome.js',
		_esPromisePollyfill = 'https://cmxz.top/Bot/js/es6-promise.min.js',
		_searchselect = "https://cmxz.top/Bot/js/vue-select.js";
		...
		function _parseMarkDown(text) {
			return text.replace(_markDownRegex.image, "<img class='botui-message-content-image' src='$2' alt='$1' />").replace(_markDownRegex.icon, "<i class='botui-icon botui-message-content-icon fa fa-$1'></i>").replace(_markDownRegex.link, _linkReplacer);
		}
		...
		var _instance = _botApp.$children[0];
		...
		if (_options.searchselect) {
			loadScript(_searchselect,
			function() {
				Vue.component('v-select', VueSelect.VueSelect);
			});
		}
		if (_options.debug) {
			_interface._botApp = _botApp;
		}
		return _interface;
	});
	return BotUI;
